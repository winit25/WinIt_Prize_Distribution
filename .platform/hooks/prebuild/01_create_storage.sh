...
# Double-check views directory exists and is writable
if [ ! -d "$STAGING_DIR/storage/framework/views" ]; then
    echo "ERROR: views directory still doesn't exist, creating again..."
    mkdir -p $STAGING_DIR/storage/framework/views
    chmod 777 $STAGING_DIR/storage/framework/views
    chown webapp:webapp $STAGING_DIR/storage/framework/views || true
fi
