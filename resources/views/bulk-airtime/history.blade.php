...
@section('title', 'Airtime Batch History - WinIt Prize Distribution')
...
<div class="container-fluid">
    <div class="batch-history-card">
        <div class="batch-header">
            <h2>
                <i class="fas fa-history me-2"></i>Airtime Batch History
            </h2>
            <div>
                <a href="{{ route('bulk-airtime.index') }}" class="btn btn-primary">
        ...
        <div class="card-body p-4">
            ...
            <div class="batch-filters">
                ...
                <!-- Search Form -->
                <form id="searchFilterForm" method="GET" action="{{ route('bulk-airtime.history') }}" class="search-filter-form">
                    <div class="row g-3 mb-3 align-items-end">
                        ...
                        <div class="col-md-3 d-flex align-items-end gap-2">
                            ...
                            <a href="{{ route('bulk-airtime.history') }}" class="btn btn-outline-secondary btn-sm">
                ...
                <div class="filter-buttons">
                    <a href="{{ route('bulk-airtime.history', array_merge(request()->except('status', 'page'), ['status' => ''])) }}" 
                       class="filter-btn {{ !request('status') ? 'active' : '' }}" 
                       data-filter="all">
                        <i class="fas fa-list"></i>All ({{ $batches->total() }})
                    </a>
                    <a href="{{ route('bulk-airtime.history', array_merge(request()->except('status', 'page'), ['status' => 'uploaded'])) }}" 
                    ...
                    <a href="{{ route('bulk-airtime.history', array_merge(request()->except('status', 'page'), ['status' => 'processing'])) }}" 
                    ...
                    <a href="{{ route('bulk-airtime.history', array_merge(request()->except('status', 'page'), ['status' => 'completed'])) }}" 
                    ...
                    <a href="{{ route('bulk-airtime.history', array_merge(request()->except('status', 'page'), ['status' => 'failed'])) }}" 
                                ...
                                <tr data-status="{{ $batch->status ?? 'uploaded' }}" data-id="{{ $batch->id }}" style="transition: all 0.3s ease;">
                                    ...
                                    <td>
                                        <div class="batch-actions">
                                            <a href="{{ route('bulk-airtime.show', $batch->id) }}" class="action-btn view" title="View Details">
                                            ...
                                            @if($batch->status == 'completed' || $batch->status == 'failed' || $batch->processed_recipients > 0)
                                                <a href="{{ route('bulk-airtime.download-report', $batch->id) }}" class="action-btn download" title="Download Batch Report">
...
function checkProcessingBatches() {
    ...
    if (processingRows.length > 0) {
        ...
        setTimeout(() => {
            if (window.location.pathname.includes('/bulk-airtime/history')) {
...
function processBatch(batchId) {
    ...
    .then(data => {
        ...
        setTimeout(() => {
            window.location.href = `/bulk-airtime/show/${batchId}`;
        }, 2000);
    })
    .catch(error => {
        modal.hide();
...
function downloadReport(batchId) {
    window.location.href = `/bulk-airtime/download-report/${batchId}`;
}
