...
@section('title', 'DSTV Batch History - WinIt Prize Distribution')
...
<div class="container-fluid">
    <div class="batch-history-card">
        <div class="batch-header">
            <h2>
                <i class="fas fa-history me-2"></i>DSTV Batch History
            </h2>
            <div>
        ...
        <div class="card-body p-4">
            ...
            <div class="batch-filters">
                ...
                <!-- Search Form -->
                <form id="searchFilterForm" method="GET" action="{{ route('bulk-dstv.history') }}" class="search-filter-form">
                    <div class="row g-3 mb-3 align-items-end">
                        ...
                        <div class="col-md-3 d-flex align-items-end gap-2">
                            ...
                            <a href="{{ route('bulk-dstv.history') }}" class="btn btn-outline-secondary btn-sm">
                ...
                <div class="filter-buttons">
                    <a href="{{ route('bulk-dstv.history', array_merge(request()->except('status', 'page'), ['status' => ''])) }}" 
                       class="filter-btn {{ !request('status') ? 'active' : '' }}" 
                       data-filter="all">
                        <i class="fas fa-list"></i>All ({{ $batches->total() }})
                    </a>
                    <a href="{{ route('bulk-dstv.history', array_merge(request()->except('status', 'page'), ['status' => 'uploaded'])) }}" 
                    ...
                    <a href="{{ route('bulk-dstv.history', array_merge(request()->except('status', 'page'), ['status' => 'processing'])) }}" 
                    ...
                    <a href="{{ route('bulk-dstv.history', array_merge(request()->except('status', 'page'), ['status' => 'completed'])) }}" 
                    ...
                    <a href="{{ route('bulk-dstv.history', array_merge(request()->except('status', 'page'), ['status' => 'failed'])) }}" 
                                ...
                                <tr data-status="{{ $batch->status ?? 'uploaded' }}" data-id="{{ $batch->id }}" style="transition: all 0.3s ease;">
                                    ...
                                    <td>
                                        <div class="batch-actions">
                                            <a href="{{ route('bulk-dstv.show', $batch->id) }}" class="action-btn view" title="View Details">
                                            ...
                                            @if($batch->status == 'completed' || $batch->status == 'failed' || $batch->processed_recipients > 0)
                                                <a href="{{ route('bulk-dstv.download-report', $batch->id) }}" class="action-btn download" title="Download Batch Report">
...
function checkProcessingBatches() {
    ...
    if (processingRows.length > 0) {
        ...
        setTimeout(() => {
            if (window.location.pathname.includes('/bulk-dstv/history')) {
...
function processBatch(batchId) {
    ...
    utils.makeRequest(`/bulk-dstv/process/${batchId}`, {
        method: 'POST'
    })
    .then(data => {
        modal.hide();
        ...
        setTimeout(() => {
            window.location.href = `/bulk-dstv/show/${batchId}`;
        }, 2000);
    })
    .catch(error => {
        modal.hide();
...
function downloadReport(batchId) {
    window.location.href = `/bulk-dstv/download-report/${batchId}`;
}
